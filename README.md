...
## The scary part
Since this classifier outputs a probability that the user is doing some activity, the prediction of this classifier can be used in conjuction with a state transition model (Markov transition matrix) to predict the most likely sequence of activities in real-time. Moreover, a more accurate inference can be drawn about the most likely sequence of activities by using a [forward-backward algorithm](https://en.wikipedia.org/wiki/Forward%E2%80%93backward_algorithm) from a longer train of accelerometer data.
